...
```powershell
Compress-Archive -LiteralPath manifest.json,content.js,reader.css,src -DestinationPath fanqie-reader-helper.zip -Force
```

## Privacy

本擴展只使用 `storage` 權限保存本地閱讀設定，例如字體大小、行距、頁面寬度、縮放比例、主題、論文模式開關和插圖模式開關。
